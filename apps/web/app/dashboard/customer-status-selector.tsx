...
export function CustomerStatusSelector({
	hideTitle = false,
	collapsed = false,
}: {
	hideTitle?: boolean
	...
	return (
		<div className='space-y-2'>
			{!hideTitle && (
				<div className='h-6'>
					<div className={cn('hidden px-4 font-title text-muted-foreground', !collapsed && 'block')}>
						Customer status
					</div>
				</div>
			)}
			<Button
				...
				className={cn('flex gap-4 text-muted-foreground', !collapsed && 'w-full justify-start')}
				asChild
			>
				<Link href='/dashboard'>
					...
					<span className={cn('hidden', !collapsed && 'inline')}>Waiting for help</span>
				</Link>
			</Button>
			<Button
				size={collapsed ? 'icon' : 'default'}
			...
			>
				<Link href='/dashboard?status=helping'>
					...
					<span className={cn('hidden', !collapsed && 'inline')}>Helping</span>
				</Link>
			</Button>
			<Button
				size={collapsed ? 'icon' : 'default'}
			...
			>
				<Link href='/dashboard?status=helped'>
					...
					<span className={cn('hidden', !collapsed && 'inline')}>Helped</span>
				</Link>
			</Button>
			<Button
				size={collapsed ? 'icon' : 'default'}
			...
			>
				<Link href='/dashboard?status=spam'>
					...
					<span className={cn('hidden', !collapsed && 'inline')}>Spam</span>
				</Link>
			</Button>
		</div>
	)
